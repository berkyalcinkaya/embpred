...
# Write a demo where the user specifies paths to three images from the CLI and runs inference on them
...
if __name__ == "__main__":
    ...
    group.add_argument(
        "--single-image",
        type=str,
        help="Path to a single image. The image will be duplicated across all three focal depths as an RGB image."
    )
    ...
    if args.single_image:
        ...
        if single_image is None:
            ...
            exit(1)
        # Duplicate the grayscale image into RGB by stacking
        ...
        # Ensure all three paths are provided
        if not all([args.F_neg15, args.F0, args.F15]):
            print("Error: When not using --single-image, all three focal depth images (--F_neg15, --F0, --F15) must be provided.")
            exit(1)
